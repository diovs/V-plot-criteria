...
awk '{print $1,$2-12,$3+12,$4,$5,$6}' OFS='\t' CTCF_loMNase_no_bind.motif | sort -k1,1 -k2,2n -S 100%| \
 bedtools getfasta -fi /ssd/index/bismark/hg38XY/hg38XY.fa -bed - -s -name -tab \
  > CTCF_loMNase_no_bind.fa

awk '{print $1,$2-12,$3+12,$4,$5,$6}' OFS='\t' CTCF_loMNase_all_bind.motif | sort -k1,1 -k2,2n -S 100%| \
 bedtools getfasta -fi /ssd/index/bismark/hg38XY/hg38XY.fa -bed - -s -name -tab \
  > CTCF_loMNase_all_bind.fa
