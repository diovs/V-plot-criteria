...
for (i in 1:nrow(result_list_1)) {
  ...
  if (nrow(temp)>200000) {
    ...
    p[[i]]<-ggplot(data =temp, aes(x=distance, y=fragment)) + 
      ...
      #geom_point(alpha = 0.3, size = 2, color = "dodgerblue4",shape=16) +
  ...
  ggsave(paste("/mnt/disk2/2/zqf/private/GB_Vplot_Correspondence/DNase_selected/",result_list_1$TF[i],"_V_edge.png",sep = ""),p[[i]],width=4.5, height=3.5,dpi=300)
