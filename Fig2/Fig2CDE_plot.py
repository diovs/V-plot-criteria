#!/usr/bin/env python3
"""Draw the revised Fig. 2C-E Method-B discriminant scatter panels.

This Fig2-local version uses the reviewed 10-TF CSV tables copied into this
folder and preserves the label-placement logic of the source Method-B script.
"""
...
PANEL_DATA = {
    "loMNase": ROOT / "Fig2C_plot_data_loMNase_10TF.csv",
    "DNase": ROOT / "Fig2D_plot_data_DNase_10TF.csv",
    "ATAC": ROOT / "Fig2E_plot_data_ATAC_10TF.csv",
}
PANEL_OUTPUT_PREFIX = {
    "loMNase": "Fig2C_loMNase_discriminant_scatter",
    "DNase": "Fig2D_DNase_discriminant_scatter",
    "ATAC": "Fig2E_ATAC_discriminant_scatter",
}
TOP_N_PER_ASSAY = 12
...
def plot_assay(plot_df: pd.DataFrame, assay: str, thr_w: float, thr_e: float) -> None:
    ...
    stat_text = ax.text(
        ...
        (
            ...
            f"Width: p = {format_p(p_w)}\n"
            f"V$_{{in}}$ / V$_{{out}}$: p = {format_p(p_e)}"
        ),
    ...
    out_prefix = PANEL_OUTPUT_PREFIX[assay]
    fig.savefig(OUTDIR / f"{out_prefix}.png", dpi=300)
    fig.savefig(OUTDIR / f"{out_prefix}.pdf")
    fig.savefig(OUTDIR / f"{out_prefix}.svg")
    plt.close(fig)


def _upstream_selection_main() -> None:
...
def apply_10tf_label_overrides() -> None:
    TF_LABEL_POSITIONS["ATAC"].update(
        {
            "NFIC": (52.0, 1.70),
            "CEBPG": (49.3, 1.56),
            "JUND": (47.2, 1.33),
...
def plot_single(assay: str) -> pd.DataFrame:
    plot_df = pd.read_csv(PANEL_DATA[assay])
    stats_df, thr_w, thr_e = stats_table(plot_df, assay)
    plot_assay(plot_df, assay, thr_w, thr_e)
    return stats_df
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        description="Draw revised Fig. 2C-E Method-B discriminant scatter panels."
    )
    parser.add_argument(
        "--assay",
        choices=ASSAYS,
        help="Draw only one assay/panel. By default all three panels are drawn.",
    )
    args = parser.parse_args(argv)

    apply_10tf_label_overrides()
    assays = [args.assay] if args.assay else list(ASSAYS)
    stats_rows = [plot_single(assay) for assay in assays]
    if len(assays) == len(ASSAYS):
        pd.concat(stats_rows, ignore_index=True).to_csv(
            ROOT / "Fig2CDE_methodB_separation_stats_10TF_recomputed.csv",
            index=False,
        )
    print("Wrote: " + ", ".join(PANEL_OUTPUT_PREFIX[assay] for assay in assays))
